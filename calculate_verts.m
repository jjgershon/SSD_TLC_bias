...
function [ percentile_errors_BER, total_errors_BER] = calculate_verts(Mat1, Mat2, Mat3, percentile,num_of_skips, PageType, page_size)

[iterations, num_of_pages] = size(Mat1);

if( strcmp(PageType, 'L') || (strcmp(PageType,'H1') && num_of_pages == 128)  )
    irelevant_pages_num = 0;
elseif( strcmp(PageType,'H1') || strcmp(PageType,'H2') )
    irelevant_pages_num =  floor(num_of_pages*(num_of_skips/(num_of_skips+1)));
elseif( strcmp(PageType,'T') )
    irelevant_pages_num = floor(128*(num_of_skips/(num_of_skips+1)));    
end

relevant_pages_num  =  num_of_pages - irelevant_pages_num;

UniformQuantizationDelta = 25; %Taking the data of #UniformQuantizationDelta iterations
